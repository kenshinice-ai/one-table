...
> 定稿供投递用的是英文版（第一节，可直接排版成 PDF）。第二节是给我们自己的中文使用说明——**不随材料外发**。
...
**The guest journey (60 seconds, no app install):**

1. Scan a QR (poster, kiosk, or centre app) → set guests, budget, courses, dietary needs.
2. A complete, balanced menu appears instantly — starters to dessert, costed per person.
3. One tap merges every ingredient into a single shopping list, scaled to the party size.
4. **The list becomes a route: each ingredient mapped to a tenant, ordered level by level, saved to the phone as a route card** — works offline in the basement.
5. "Rather book a table?" — the same menu routes to your restaurants instead.

**Why this, and not another AI kiosk:**
...
- Cross-tenant visitation by design — every route is a multi-tenant journey.
- A seasonal campaign engine: Lunar New Year reunion dinner, Mid-Autumn, Christmas — new menus per season under your brand.
- Weekly aggregate demand insights (top cuisines, ingredients, dietary filters). **No accounts, no personal data collected — privacy by design.**
- A tenant-relations story: measurable traffic delivered to fresh-food and dining tenants.

**Formats:** white-label QR campaign · touch-kiosk mode · embed in centre app/site. Your logo, your colours, your tenants.
...
**为什么导航是头条**：现有系统的旅程终点是食谱二维码；我们的终点是顾客带着路线卡走完 6 家店。前者是内容，后者是客流——商场买单的是客流。整页 pitch 的每一段都要能回到这句话。

**演示准备**（见技术清单 W5）：demo 用虚构的「Sample Centre」楼层图，绝不用任何真实商场的平面图和商标。

**定价弹性**：$3,500 是锚点；对首个签约客户可给到 $2,500「创始合作价」换 logo 使用权（case study 授权写进合同）。
