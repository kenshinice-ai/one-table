...
**原则：完美体验第一，不为体积做任何妥协。** 目标设备是性能过剩的手机 / iPad，网络是 5G。历史上那套「首屏 JS ≤ 250KB」之类的数字预算已作废——它会诱导我们删掉本该呈现的内容。这里只保留**读者能感知的**指标。

| 体验指标 | 要求 |
| --- | --- |
| 打开页面看到完整一桌菜 | 立即，0 次网络往返、0 骨架 |
| 点开菜谱看到食材与步骤 | 立即，不出现 loading |
| 改条件 / 换一组 / 换这道 | 立即，纯本地计算 |
| 打印、购物清单、存为图片 | 立即，无服务端往返 |
| 布局稳定（CLS） | 无跳动：图片带显式尺寸，占位保持版面 |
| 分享链接复现 | 同链接必得同一桌菜 |

**为达成这些体验所做的事**（顺带也更快，但快不是目的）
- **默认菜单构建期算好、直接写进 HTML**：打开即是四道真菜——菜名、图片、价格、热量、健康指数俱全。完整目录在后台并行取回后接管交互。
- **所有数据在读者需要之前就已到位**：目录负载 `rel="preload"`，详情负载紧随其后立即预取。点开任何一道菜时步骤已在本地。
- **首屏菜品图由浏览器预加载**，并提供 320/640/1280 三档，让每个视口都取到合适尺寸而不是硬缩一张大图。
- **全部计算在本地**：排菜、筛选、换算、购物清单合并、分享卡渲染都不发请求，因此永远没有「转圈」。
- 唯一会短暂显示骨架的情况：打开一条**带条件的分享链接**——它描述的是另一桌菜，必须等目录到达才能如实呈现。

**明确不做**
- 不设 KB 上限，不为压缩体积删减任何已呈现的内容。
- 列表虚拟化、SSR 流式改造——当前规模下只增加复杂度与出错面。
